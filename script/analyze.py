...
def entropy(Y, probs=False):
    ...
    if probs:
        if np.sum(Y) != 1:
            if np.sum(Y) == 0:
                # print(f'Warning for {Y} and sum {np.sum(Y)}, returning None')
                return None
            else:
        ...
        return np.sum((-1) * Y * np.log10(Y + np.finfo(float).eps))
    else:
        _, count = np.unique(Y, return_counts=True, axis=0)
        prob = count/len(Y)
    
        return np.sum((-1) * prob * np.log10(prob + np.finfo(float).eps))
...
def simulate_contigency_table(ct, ct_type):
    """
...
def find_optimal_threshold(sim_null, sim_proper):

    def add_noise(data, noise_level=1e-6):
            
        data = np.where(np.isinf(data), np.finfo(np.float64).max, data)
        
        data = np.where(np.isinf(data), np.finfo(np.float64).min, data)
        
        data = np.where(np.isnan(data), 0, data)
        return data + np.random.normal(0, noise_level, data.shape)
    
    kde_null = gaussian_kde(add_noise(sim_null))
    kde_proper = gaussian_kde(add_noise(sim_proper))

    min_range = min(sim_null.min(), sim_proper.min())
    max_range = max(sim_null.max(), sim_proper.max())
    x = np.linspace(min_range, max_range, 1000)

    mode_null = x[np.argmax(kde_null(x))]
    mode_proper = x[np.argmax(kde_proper(x))]

    if mode_null > mode_proper:
        mode_null, mode_proper = mode_proper, mode_null

    thresholds = np.linspace(mode_null, mode_proper, 1000)
    min_sum_errors = float('inf')
    optimal_threshold = None

    # Iterate over thresholds to find the one that minimizes the sum of Type I and II errors
    for threshold in thresholds:
        type_1_error = np.min([(sim_null >= threshold).mean(), (sim_null <= threshold).mean()])   # False positive rate
        type_2_error = np.min([(sim_proper <= threshold).mean(), (sim_proper >= threshold).mean()])  # False negative rate
        sum_errors = type_1_error + type_2_error
        if sum_errors < min_sum_errors:
            min_sum_errors = sum_errors
            optimal_threshold = threshold

    return min_sum_errors, optimal_threshold


# In[ ]:


def add_min_sum_errors(sim_null, sim_proper, quantiles, alpha, save=False, save_dir=None, file_type='xlsx'):
    for idx in sim_null.index:
        ...
        quantiles.loc[idx, 'optimal_threshold'] = optimal_threshold
...
def simulation_hist(sim_null, ct_null,
    ...
    for idx, ((index_null, row_null), (index_proper, row_proper)) in enumerate(zip(sim_null.iterrows(), sim_proper.iterrows())):
        ...
        # 'null' data
        sns.histplot(row_null, bins=10, kde=True, ax=ax_null, stat='density', legend=False)
        ...
        # 'proper' data
        sns.histplot(row_proper, bins=10, kde=True, ax=ax_proper, stat='density', legend=False)
...
def binary_bipartite_network_clusterplot(df, response_col,  method='ward', metric='euclidean',
                                          show_all_labels=False, return_clusters=False, t=None, ct_type='null',
                                            n_clusters=None, save_dir=None, figsize=(10, 15), show_plot=False,
                                              save=False, title=None, file_type = 'pdf', flip=False):
    if df.shape[1] <= 1:
    ...
    if df_.columns.unique().shape[0] >= 2:
        print(f"Warning: df has {df_.columns.unique().shape[0]} unique columns i.e. {df_.columns.unique()}.")
    try:
        categories = df[response_col].unique()
    except AttributeError as e:
        print(f"Warning: response_col {response_col} gave error {e}.")
        categories = df[response_col].dropna().drop_duplicates()
    ...
    g = sns.clustermap(df_, method=method, metric=metric, cmap='flare', figsize=figsize, col_colors=col_colors, tree_kws=dict(linewidths=2.3))
    ...
    for fig_num in range(num_figures):
        ...
        for ax_index in range(nrows * ncols):
            if plot_count < num_plots:
                ...
                # Assuming sim_null and sim_proper are your DataFrames and plot_count is defined
                noise_std_dev = 0.01  # Standard deviation of the noise

                # Add noise to your data
                sim_null_noisy = sim_null.copy()
                sim_null_noisy.iloc[plot_count, :] += np.random.normal(0, noise_std_dev, sim_null.shape[1])

                sim_proper_noisy = sim_proper.copy()
                sim_proper_noisy.iloc[plot_count, :] += np.random.normal(0, noise_std_dev, sim_proper.shape[1])

                sns.histplot(sim_null_noisy.iloc[plot_count, :], kde=True, bins='sqrt', color="blue", stat="density", alpha=0.5, ax=ax)
                sns.histplot(sim_proper_noisy.iloc[plot_count, :], kde=True, bins='sqrt', color="red", stat="density", alpha=0.5, ax=ax)
...
#                 filter_response=False, valid_response_list=None, save=False, save_dir=None, generate_files_only=False,
#                    use_checkpoint=True, file_type='xlsx', only_sigf=False, show_all_labels=False):
    
#     df_ = df.copy(deep=True)

#     if filter_response and valid_response_list is not None:
#         df_ = df_[df_[response_col].isin(valid_response_list)]
...
#     print('===========================================================')
#     print(df_[response_col].value_counts().to_frame())
#     print("\n\n")
...
#     bb_net_null_list = []
#     print_freq = len(pred_cols) % 50
#     for idx, pred_col in tqdm(enumerate(sorted(pred_cols)), desc='Processing:', total=len(pred_cols)):
#         gc.collect()
...
#                     if idx % print_freq == 0:
#                         print(f"Skipping {pred_col} via checkpoint")
#                     continue
#                 if idx > 0:
#                     bb_net_null_list.append(pd.read_excel(filepath, index_col=0, engine='openpyxl').iloc[:, 1:] )
#                 else:
#                     bb_net_null_list.append(pd.read_excel(filepath, index_col=0, engine='openpyxl'))
...
#         sim_df_null = ensemble_ct( ct_df[0], n_samples = n_samples, ct_type=ct_type)
#         ct_ci_df_null = ensemble_ct_CI(sim_df_null, ct_df[0], alpha=alpha,ct_type=ct_type, file_type=file_type,
#                                 save=False, save_dir=save_dir_)


#         ct_type = 'proper'

#         sim_df_proper = ensemble_ct( ct_df[0], n_samples = n_samples, ct_type=ct_type)
...
#         ct_ci_df_null = add_min_sum_errors(sim_df_null, sim_df_proper, ct_ci_df_null, alpha=alpha, save=save, save_dir=save_dir_, file_type='xlsx')
...
#         if len(bb_net_null_list) > 0:
#             bb_net_null = bb_net_null.reset_index(drop=True).iloc[:, 1:]
#         else:
#             bb_net_null = bb_net_null.reset_index(drop=True)
#         bb_net_null_list.append(bb_net_null)
#         plt.close('all')
...
#     bb_net_null_df.index = bb_net_null_df.index + 1
...
#         bb_net_null_df.to_excel(f'{save_dir}/bipartite_network_summary.xlsx')
...
def combine_cols(df, cols, new_col_name, sep, categorical, bin=None):
    ...
    if not isinstance(cols, list):
        cols = list(cols)
    # print("\n==========Debug==============\n ")
    # print(f" df.columns.tolist(): {df.columns.tolist()[:5]}")
    # print(f'cols: {cols}')
    # print(f'new_col_name: {new_col_name}')
    # print(f'sep: {sep}')
...
def run_experiment(df, pred_cols, response_col, n_samples, alpha, sigf_col, do_summary=True,
                save=False, save_dir=None, generate_files_only=False,
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_pred_col, pred_col, df, response_col, n_samples, alpha, sigf_col,
                                                    save, save_dir, file_type, only_sigf, use_checkpoint,
                                                    generate_files_only): pred_col for pred_col in pred_cols}
...
# In[ ]:
    ...
    if combine:
        if categorical:
            save_dir = os.path.join(args.save_dir, 'combine_cat', str(num_combine) + '_features_combinations', response_filter_str)
        else:
            save_dir = os.path.join(args.save_dir, 'combine_raw', str(num_combine) + '_features_combinations', response_filter_str)
    else:
        if categorical:
            save_dir = os.path.join(args.save_dir, '1_feature_cat', response_filter_str)
        else:
            save_dir = os.path.join(args.save_dir, '1_feature_raw', response_filter_str)
...
# In[ ]:
    ...
    run_experiment(df=transformed_data_df, pred_cols=pred_cols, response_col=response_col, n_samples=n_samples,
                    alpha=alpha, sigf_col=sigf_col, do_summary=do_summary, save=save, save_dir=save_dir,
